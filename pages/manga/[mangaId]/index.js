...
const BannerGradient = styled("div")({
  ...
  top: 0,
  left: 0,
  background: "linear-gradient(180deg,rgb(0 0 0 / 0%) 0,#fff)",
});
...
const Home = () => {
  ...
  useEffect(() => {
    if(!router.query.tab) {
      router.push('/manga/' + router.query.mangaId +'/?tab=0', undefined, { shallow: true })
    }
  }, [])
  if (loading) {
    return <>loading</>;
        ...
        <Grid item xs={9}>
          <Box>
            ...
            <Box sx={{ mt: 1.5, display: "flex", gap: 1 }}>
              ...
              <Button
                variant="outlined"
                startIcon={<Icon icon={bookOpenFill} width={20} height={20} />}
              >
                {"Start Reading"}
              </Button>
          ...
          <Typography variant="subtitle1" component="p" sx={{ my: 2, p: 1, fontWeight: "400" }}>
            {manga.data.attributes.description}
          </Typography>
          <Tabs onChange={(_, value) => router.push('/manga/' + router.query.mangaId +'/?tab=' + value, undefined, { shallow: true })} value={Number(router.query.tab)}>
            {
              ["Chapters", "Comments", "Art", "Related", "Persons"].map(label => (
          ...
          <Divider />
          {[
            <ChapterList key={0} chapters={manga.data.attributes.chapters.data} />,
            <Comments  key={1}/>,
